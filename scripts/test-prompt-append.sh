...
#   - /tmp/fragment.v3.jsonc          current fragment snapshot (dynamic — recreated from $FRAGMENT at test start)
...
test_S3() {
  ...
  # byte-equals fragment (current fragment byte-equals itself — dynamic snapshot invariant).
  local id=S3 log="$LOG_DIR/test-S3.log"
  {
    ...
    # This is the dynamic-snapshot invariant — preserved across re-runs since SNAP_V3 is taken fresh from current FRAGMENT at test start.
    for agent in prometheus sisyphus atlas; do
      local expected now
...
test_S5() {
  local id=S5 log="$LOG_DIR/test-S5.log" rc rc2
  {
    ...
    # Sanity: count agents keys in current fragment (defensive — should be 3)
    local fragment_keys
    fragment_keys=$(jq_jsonc "$FRAGMENT" '.agents | keys | sort | length')
    echo "fragment key count=$fragment_keys"
...
EOF
    ...
    # Restore fragment + re-install SNAP_V3 state so workspace ends in canonical (dynamic-snapshot) state
    restore_fragment || true
    ...
    if [ "$rc2" -ne 0 ]; then
      fail "$id" "post-restore SNAP_V3 install exit=$rc2 (expected 0)"
      return 1
    fi
...
# Snapshot current fragment (always from current $FRAGMENT) into SNAP_V3 at test start
