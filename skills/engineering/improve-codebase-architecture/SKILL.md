...
- **User rejects the candidate with a load-bearing reason?** Call the Skill tool with `architecture-decision-records` to record it, framed as: _"Want me to capture this as an ADR so future architecture reviews don't re-suggest it?"_ Only offer when the reason would actually be needed by a future explorer to avoid re-suggesting the same thing; skip ephemeral reasons ("not worth it right now") and self-evident ones. Do **not** write ADR files directly from this skill — `architecture-decision-records` is the only owner of `docs/adr/`.
