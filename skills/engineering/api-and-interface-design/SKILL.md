...
description: "Design stable, hard-to-misuse APIs and module interfaces (REST, GraphQL, type contracts, component props, seams). Use when: defining new endpoints, setting module boundaries, designing public types or schemas, / design an interface, / API contract, / module boundary, / 接口契约. Apply Hyrum's Law, the One-Version Rule, and contract-first design. Significant interface contracts (cross-team / public surface / breaking-change-prone) should be recorded as ADRs in `docs/adr/` via `architecture-decision-records`; routine module-internal contracts land in `docs/agents/<module>.md`. NOT for deep-module design vocabulary itself (use `codebase-design`) or for ADR authoring (use `architecture-decision-records`)."
---
...
- **Plan for deprecation at design time.** Capture significant interface contracts as ADRs via `architecture-decision-records` (lifecycle: Proposed → Accepted → Deprecated → Superseded); retiring an interface is a lifecycle change, not a silent removal.
